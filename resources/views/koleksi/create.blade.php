...
<div class="flex items-center justify-between flex-wrap gap-3">
    <div>
        ...
        <h1 class="text-xl font-bold text-gray-900">Tambah Koleksi Baru</h1>
    </div>
</div>
@endsection
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
    <form method="POST" action="{{ route('koleksi.store') }}" id="form-koleksi" enctype="multipart/form-data" class="p-6">
        @csrf
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="judul_koleksi2" value="{{ old('judul_koleksi2') }}"
            ...
            <div>
                ...
                <select name="kategori"
                    ...
                    <option value="1" {{ old('kategori') == '1' ? 'selected' : '' }}>Koleksi Terbaru</option>
                    <option value="2" {{ old('kategori') == '2' ? 'selected' : '' }}>Koleksi Populer</option>
                    <option value="3" {{ old('kategori') == '3' ? 'selected' : '' }}>Koleksi Referensi</option>
                    <option value="4" {{ old('kategori') == '4' ? 'selected' : '' }}>Informasi Terkini</option>
                </select>
                @error('kategori')
                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                @enderror
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-sky-300 transition-colors">
                    ...
                    <label for="foto_koleksi2" class="cursor-pointer">
                        <div id="preview-container" class="hidden">
                            <img id="image-preview" src="" alt="Preview" class="max-h-64 mx-auto rounded-lg shadow-sm mb-3">
            ...
            <div>
                ...
                <textarea name="isi_koleksi" rows="6"
                          ...
                          class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm
                                 focus:outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-colors
                                 @error('isi_koleksi') border-red-300 @enderror">{{ old('isi_koleksi') }}</textarea>
                @error('isi_koleksi')
                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
