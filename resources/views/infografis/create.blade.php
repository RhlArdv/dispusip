...
<div class="flex items-center gap-4 mb-6">
    ...
    <div>
        <h1 class="text-xl font-bold text-gray-900">Tambah Infografis</h1>
        <p class="text-[13px] text-gray-500 mt-0.5">Tambah infografis baru untuk carousel hero E-Perpus</p>
    </div>
</div>
@endsection
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <form action="{{ route('infografis.store') }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
        @csrf
        ...
        <div class="max-w-2xl space-y-6">
            ...
            <!-- Gambar -->
            <div>
                <label for="image" class="block text-sm font-medium text-gray-700 mb-1">Gambar Infografis (Landscape 16:9 disarankan)</label>
                <input type="file" name="image" id="image" accept="image/*"
                       class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100" required>
                @error('image')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
