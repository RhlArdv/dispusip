...
@section('content')
    ...
    <div class="relative w-full h-[60vh] md:h-[80vh] bg-navy-950 overflow-hidden mt-20 group" x-data="{
        ...
        @if($infografis->count() > 0)
            @foreach($infografis as $index => $info)
                <div class="absolute inset-0 transition-opacity duration-1000 ease-in-out bg-navy-950" x-show="activeSlide === {{ $index }}"
                    ...
                    <img src="{{ Storage::url($info->image) }}" class="absolute inset-0 w-full h-full object-cover blur-2xl opacity-50 scale-110" alt="">
                    
                    <!-- Image with original proportions (object-contain) to prevent zooming -->
                    <img src="{{ Storage::url($info->image) }}" alt="{{ $info->title }}" class="relative w-full h-full object-contain">
                    
                    <!-- Subtle bottom gradient only for indicator visibility -->
                    <div class="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-navy-900/90 to-transparent opacity-80 z-10"></div>
                </div>
            @endforeach
        @else
    ...
    <div class="relative bg-fixed bg-bottom bg-no-repeat" style="background-image: url('{{ asset('assets/img/backgroudndisp.svg') }}'); background-size: 100% auto;">

    <!-- LAYANAN PERPUSTAKAAN SECTION -->
    <section class="py-16 px-6 bg-white/60 border-t border-white/50">
        <div class="max-w-7xl mx-auto">
            <div class="text-center mb-12">
                <h2 class="text-3xl md:text-4xl font-black text-navy-900 tracking-tight uppercase">Layanan Perpustakaan</h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <a href="/#faq"
                    ...
                    <h3 class="text-lg font-bold text-navy-900 group-hover:text-gold-600 transition-colors">FAQ Perpustakaan
                    </h3>
                        ...
                        <a href="{{ route('public.berita.show', $berita->slug) }}"
                            ...
                            <div class="p-8 flex-1 flex flex-col">
                                ...
                                <h3
                                    ...
                                    {{ $berita->judul_berita }}</h3>
                                <p class="text-navy-600/80 line-clamp-3 text-sm leading-relaxed mb-6 flex-1">
                                    {{ Str::limit(strip_tags(html_entity_decode($berita->isi_berita)), 120) }}</p>
                            </div>
                        </a>
                    @empty
            ...
            <div class="max-w-7xl mx-auto px-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @forelse($testimoni as $testi)
                        <div class="bg-white/70 p-6 rounded-3xl border border-white/50 relative shadow-md hover:shadow-xl transition-all duration-300 flex flex-col">
                            @if($testi->youtube_id)
                                <div class="w-full aspect-video rounded-2xl overflow-hidden mb-5 shadow-lg bg-navy-900 border border-white/20">
                            ...
                            @else
                                <div class="w-full aspect-video rounded-2xl overflow-hidden mb-5 shadow-lg bg-navy-900 border border-white/20 flex items-center justify-center">
                    ...
                    @empty
                        ...
                        @for($i = 1; $i <= 3; $i++)
                            <div class="bg-white/70 p-6 rounded-3xl border border-white/50 relative shadow-md">
                                <div class="w-full aspect-video rounded-2xl overflow-hidden mb-5 shadow-lg bg-navy-900 flex items-center justify-center relative group cursor-pointer border border-white/20">
                                    <img src="https://images.unsplash.com/photo-1507842217343-583bb7270b66?auto=format&fit=crop&q=80&w=800" class="w-full h-full object-cover opacity-50 group-hover:scale-105 transition-transform duration-700">
                                    <div class="absolute w-14 h-14 bg-red-600 rounded-full flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition-transform">
                ...
                <div class="mt-12 text-center">
                    <a href="{{ route('public.testimoni.index') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-white border border-navy-100 text-navy-600 font-bold rounded-xl hover:bg-navy-50 hover:text-gold-600 transition-all shadow-sm">
