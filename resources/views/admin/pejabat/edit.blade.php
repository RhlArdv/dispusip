...
<div class="flex items-center gap-4">
    ...
    <div>
        <h1 class="text-xl font-bold text-gray-900">Edit Pejabat: {{ $pejabat->nama }}</h1>
        <p class="text-[13px] text-gray-500 mt-0.5">Perbarui data pejabat</p>
    </div>
</div>
@endsection
        ...
        <div class="p-6 md:p-8 space-y-6">
            ...
            @if($pejabat->image)
                <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                    <img src="{{ asset('storage/' . $pejabat->image) }}"
                         class="w-20 h-20 rounded-2xl object-cover border-2 border-white shadow-md">
                    <div>
                        <p class="text-sm font-semibold text-gray-700">Foto Saat Ini</p>
                        <p class="text-xs text-gray-400 mt-1">Upload foto baru di bawah untuk menggantinya.</p>
                    </div>
                </div>
            @endif
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3"
                          class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition-all outline-none resize-none">{{ old('keterangan', $pejabat->keterangan) }}</textarea>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                    {{ $pejabat->image ? 'Ganti Foto' : 'Upload Foto' }}
                </label>
                ...
                <p class="text-xs text-gray-500 mt-1.5">Format: JPG, PNG. Maks: 2MB.</p>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="order_no" value="{{ old('order_no', $pejabat->order_no) }}" min="0"
                           class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition-all outline-none">
                    <p class="text-xs text-gray-400 mt-1">Urut 1 = tampil paling atas (Kepala Dinas)</p>
                    @error('order_no') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Status</label>
                    <label class="inline-flex items-center gap-2 mt-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $pejabat->is_active ? 'checked' : '' }}>
            ...
            <div class="pt-2 border-t border-gray-100">
                <p class="text-sm font-semibold text-gray-700 mb-4">Media Sosial (Opsional)</p>
                <div class="space-y-3">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600 shrink-0">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                        </div>
                        <input type="url" name="facebook" value="{{ old('facebook', $pejabat->facebook) }}" placeholder="https://facebook.com/..."
                               class="flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-sky-100 flex items-center justify-center text-sky-600 shrink-0">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        </div>
                        <input type="url" name="twitter" value="{{ old('twitter', $pejabat->twitter) }}" placeholder="https://x.com/..."
                               class="flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-pink-100 flex items-center justify-center text-pink-600 shrink-0">
                        ...
                        <input type="url" name="instagram" value="{{ old('instagram', $pejabat->instagram) }}" placeholder="https://instagram.com/..."
                               class="flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
                    </div>
                </div>
            </div>
