...
<div class="p-6 lg:p-8">
    ...
    {{-- Form --}}
    <div class="max-w-4xl">
        <form action="{{ route('buku.update', $buku) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 lg:p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <select id="kategori_buku_id"
                            ...
                            @foreach($kategoriBuku as $kategori)
                                <option value="{{ $kategori->id }}" {{ $buku->kategori_buku_id == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea id="abstrak"
                              ...
                              rows="3"
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                              placeholder="Ringkasan singkat buku">{{ old('abstrak', $buku->abstrak) }}</textarea>
                    @error('abstrak')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="uraian"
                              name="uraian"
                              rows="6"
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">{{ old('uraian', $buku->uraian) }}</textarea>
                    @error('uraian')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @if($buku->sampul)
                            <div class="mt-3">
                                ...
                                <img src="{{ $buku->sampul_url }}" alt="Current cover" class="w-32 h-40 object-cover rounded-lg shadow-md">
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        @if($buku->file_pdf)
                            <div class="mt-3">
                                <p class="text-xs text-gray-500">PDF saat ini: <a href="{{ $buku->pdf_url }}" target="_blank" class="text-indigo-600 hover:underline">Lihat PDF</a></p>
                            </div>
                        @endif
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_published" value="1" {{ $buku->is_published ? 'checked' : '' }}
                               class="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        <span class="text-sm font-medium text-gray-700">Terbitkan (Published)</span>
                    </label>
                </div>
            </div>
