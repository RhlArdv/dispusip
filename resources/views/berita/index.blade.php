...
<div class="flex items-center justify-between flex-wrap gap-3">
    <div>
        <h1 class="text-xl font-bold text-gray-900">Kelola Berita</h1>
        <p class="text-[13px] text-gray-500 mt-0.5">Manajemen konten berita dan artikel</p>
    </div>
...
@section('content')
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-gray-100">
            <p class="text-[13px] text-gray-500">Daftar semua berita yang terdaftar di sistem.</p>
        </div>

        <div class="p-5">
            <table id="tabel-berita" class="w-full text-sm" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider pb-3">Judul Berita</th>
    ...
    <div id="modal-hapus"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 class="font-bold text-gray-900 mb-1">Hapus Berita?</h3>
            <p class="text-sm text-gray-500 mb-6">
                Berita <span id="nama-hapus" class="font-semibold text-gray-800"></span> akan dihapus permanen.
            </p>
...
<style>
    /* DataTables styling */
    #tabel-berita_wrapper .dataTables_length,
    #tabel-berita_wrapper .dataTables_filter {
        display: flex;
        align-items: center;
        gap: 0.5rem;
        font-size: 0.8125rem;
    ...
    #tabel-berita_wrapper .dataTables_length select {
        appearance: none;
    ...
    #tabel-berita_wrapper .dataTables_length select:focus {
        outline: none;
        border-color: #38bdf8;
        box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.15);
    }
    #tabel-berita_wrapper .dataTables_filter input {
    ...
    #tabel-berita_wrapper .dataTables_filter input:focus {
        outline: none;
        border-color: #38bdf8;
        box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.15);
    }
    #tabel-berita tbody tr { border-bottom: 1px solid #f9fafb; }
    #tabel-berita tbody tr:hover td { background: #f0f9ff !important; }
    #tabel-berita thead tr th { border-bottom: 2px solid #f3f4f6 !important; }

    #tabel-berita_wrapper .dataTables_info {
        font-size: 0.8125rem;
        color: #6b7280;
        padding-top: 0.75rem;
    }
    #tabel-berita_wrapper .dataTables_paginate {
        display: flex;
        align-items: center;
        justify-content: flex-end;
        gap: 0.25rem;
    ...
    #tabel-berita_wrapper .dataTables_paginate span {
        display: inline-flex;
        gap: 0.25rem;
    }
    #tabel-berita_wrapper .dataTables_paginate .paginate_button {
    ...
    #tabel-berita_wrapper .dataTables_paginate .previous,
    #tabel-berita_wrapper .dataTables_paginate .next {
        border: 1px solid #e5e7eb !important;
        background: #fff !important;
        margin: 0 0.125rem;
    }
    #tabel-berita_wrapper .dataTables_paginate .previous:hover:not(.disabled),
    #tabel-berita_wrapper .dataTables_paginate .next:hover:not(.disabled) {
        border-color: #38bdf8 !important;
        color: #0284c7 !important;
        background: #f0f9ff !important;
    }
    #tabel-berita_wrapper .dataTables_paginate .previous.disabled,
    #tabel-berita_wrapper .dataTables_paginate .next.disabled {
        opacity: 0.4;
        cursor: not-allowed;
    }
    #tabel-berita_wrapper .dataTables_paginate .paginate_button:not(.previous):not(.next):hover:not(.current) {
        background: #f0f9ff !important;
        color: #0284c7 !important;
        border-color: #bae6fd !important;
    }
    #tabel-berita_wrapper .dataTables_paginate .paginate_button.current,
    #tabel-berita_wrapper .dataTables_paginate .paginate_button.current:hover {
...
<script>
    ...
    let beritaIdHapus = null;

    const table = $('#tabel-berita').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
        ...
        columns: [
            ...
            { data: 'thumbnail', name: 'thumbnail', orderable: false, searchable: false, width: '100px' },
    ...
    document.getElementById('btn-konfirmasi-hapus').addEventListener('click', function () {
        ...
        .catch(() => tampilToast('error', 'Gagal menghapus berita.'))
        .finally(() => {
