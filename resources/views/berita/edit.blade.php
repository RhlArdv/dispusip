...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
    <form method="POST" action="{{ route('berita.update', $berita->id) }}" id="form-berita" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-5">
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 hover:border-sky-300 transition-colors">
                    <div class="text-center">
                        ...
                        <label for="thumbnail"
                            ...
                            <span class="text-sm text-gray-600 font-medium">Klik untuk ganti thumbnail</span>
                            <span class="text-xs text-gray-400">JPEG, PNG, JPG, GIF, Webp (Max 2MB)</span>
                        </label>

                        {{-- Preview Image --}}
                        <div id="thumbnail-preview" class="mt-4 @if($berita->thumbnail) @else hidden @endif">
                            <img src="{{ $berita->thumbnail ? asset($berita->thumbnail) : '' }}"
                                 alt="Preview"
            ...
            <div>
                ...
                <textarea name="isi_berita" id="isi_berita" rows="15"
                          ...
                          class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm
                                 focus:outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-colors
                                 @error('isi_berita') border-red-300 @enderror">{{ old('isi_berita', $berita->isi_berita) }}</textarea>
                @error('isi_berita')
                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
...
<script>
    let myEditor;
    const initialData = @json($berita->isi_berita);
