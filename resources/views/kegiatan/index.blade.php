...
<div class="flex items-center justify-between flex-wrap gap-3">
    <div>
        <h1 class="text-xl font-bold text-gray-900">Kelola Kegiatan</h1>
        <p class="text-[13px] text-gray-500 mt-0.5">Manajemen kegiatan dan aktivitas perpustakaan</p>
    </div>
...
@section('content')
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-gray-100">
            <p class="text-[13px] text-gray-500">Daftar semua kegiatan yang terdaftar di sistem.</p>
        </div>

        <div class="p-5">
            <table id="tabel-kegiatan" class="w-full text-sm" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider pb-3">Judul Kegiatan</th>
    ...
    <div id="modal-hapus"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 class="font-bold text-gray-900 mb-1">Hapus Kegiatan?</h3>
            <p class="text-sm text-gray-500 mb-6">
                Kegiatan <span id="nama-hapus" class="font-semibold text-gray-800"></span> akan dihapus permanen.
            </p>
...
<style>
    /* DataTables styling */
    #tabel-kegiatan_wrapper .dataTables_length,
    #tabel-kegiatan_wrapper .dataTables_filter {
        display: flex;
        align-items: center;
        gap: 0.5rem;
        font-size: 0.8125rem;
    ...
    #tabel-kegiatan_wrapper .dataTables_length select {
        appearance: none;
    ...
    #tabel-kegiatan_wrapper .dataTables_length select:focus {
        outline: none;
        border-color: #38bdf8;
        box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.15);
    }
    #tabel-kegiatan_wrapper .dataTables_filter input {
    ...
    #tabel-kegiatan_wrapper .dataTables_filter input:focus {
        outline: none;
        border-color: #38bdf8;
        box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.15);
    }
    #tabel-kegiatan tbody tr { border-bottom: 1px solid #f9fafb; }
    #tabel-kegiatan tbody tr:hover td { background: #f0f9ff !important; }
    #tabel-kegiatan thead tr th { border-bottom: 2px solid #f3f4f6 !important; }

    #tabel-kegiatan_wrapper .dataTables_info {
        font-size: 0.8125rem;
        color: #6b7280;
        padding-top: 0.75rem;
    }
    #tabel-kegiatan_wrapper .dataTables_paginate {
        display: flex;
        align-items: center;
        justify-content: flex-end;
        gap: 0.25rem;
    ...
    #tabel-kegiatan_wrapper .dataTables_paginate span {
        display: inline-flex;
        gap: 0.25rem;
    }
    #tabel-kegiatan_wrapper .dataTables_paginate .paginate_button {
    ...
    #tabel-kegiatan_wrapper .dataTables_paginate .previous,
    #tabel-kegiatan_wrapper .dataTables_paginate .next {
        border: 1px solid #e5e7eb !important;
        background: #fff !important;
        margin: 0 0.125rem;
    }
    #tabel-kegiatan_wrapper .dataTables_paginate .previous:hover:not(.disabled),
    #tabel-kegiatan_wrapper .dataTables_paginate .next:hover:not(.disabled) {
        border-color: #38bdf8 !important;
        color: #0284c7 !important;
        background: #f0f9ff !important;
    }
    #tabel-kegiatan_wrapper .dataTables_paginate .previous.disabled,
    #tabel-kegiatan_wrapper .dataTables_paginate .next.disabled {
        opacity: 0.4;
        cursor: not-allowed;
    }
    #tabel-kegiatan_wrapper .dataTables_paginate .paginate_button:not(.previous):not(.next):hover:not(.current) {
        background: #f0f9ff !important;
        color: #0284c7 !important;
        border-color: #bae6fd !important;
    }
    #tabel-kegiatan_wrapper .dataTables_paginate .paginate_button.current,
    #tabel-kegiatan_wrapper .dataTables_paginate .paginate_button.current:hover {
...
<script>
    ...
    let kegiatanIdHapus = null;

    const table = $('#tabel-kegiatan').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/menu/kegiatan',
    ...
    document.getElementById('btn-konfirmasi-hapus').addEventListener('click', function () {
        ...
        fetch(`/menu/kegiatan/${kegiatanIdHapus}`, {
            method: 'DELETE',
        ...
        .catch(() => tampilToast('error', 'Gagal menghapus kegiatan.'))
        .finally(() => {
