...
<div class="flex items-center justify-between flex-wrap gap-3">
    <div>
        <h1 class="text-xl font-bold text-gray-900">Tambah FAQ</h1>
        <p class="text-[13px] text-gray-500 mt-0.5">Tambahkan pertanyaan baru yang sering ditanyakan</p>
    </div>
...
<div class="max-w-4xl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <form action="{{ route('faq.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="pertanyaan" value="{{ old('pertanyaan') }}" required
                           ...
                           placeholder="Contoh: Bagaimana cara mengakses koleksi arsip?">
                    @error('pertanyaan')
                ...
                <div>
                    ...
                    <textarea name="jawaban" required rows="4"
                              class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all text-sm"
                              placeholder="Masukkan jawaban yang jelas dan padat...">{{ old('jawaban') }}</textarea>
                    @error('jawaban')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
