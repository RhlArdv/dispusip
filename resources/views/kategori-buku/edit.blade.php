...
<div class="p-6 lg:p-8">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900">Edit Kategori Buku</h1>
    </div>

    {{-- Form --}}
    <div class="max-w-2xl">
        <form action="{{ route('kategori-buku.update', $kategoriBuku) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 lg:p-8 space-y-6">
                ...
                <div>
                    ...
                    <input type="text"
                           id="nama"
                           name="nama"
                           value="{{ old('nama', $kategoriBuku->nama) }}"
                           required
                ...
                <div>
                    ...
                    <input type="text"
                           id="slug"
                           name="slug"
                           value="{{ old('slug', $kategoriBuku->slug) }}"
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                           placeholder="Contoh: fiksi, non-fiksi, anak">
                    @error('slug')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea id="deskripsi"
                              ...
                              rows="3"
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                              placeholder="Deskripsi singkat tentang kategori ini">{{ old('deskripsi', $kategoriBuku->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number"
                               id="urutan"
                               name="urutan"
                               value="{{ old('urutan', $kategoriBuku->urutan) }}"
                               min="0"
                               class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
                        @error('urutan')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="is_active"
                                name="is_active"
                                class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
                            <option value="1" {{ $kategoriBuku->is_active ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ !$kategoriBuku->is_active ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>
                </div>
            </div>
